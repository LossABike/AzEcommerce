...
function addCart(productId){
    ...
    var getSize = $('#getSize.active').attr("value");
    var qty = $('#qty_in_show').attr("value");
    var size = '';
    if(getSize != ''){
        size = getSize;
    }
        ...
        success:function(response){
                ...
                if(cartHover_existItem.length){
                    ...
                    var newItem =
                        ...
                        '<p>$' + response['cart'].price.toFixed(2)+ 'x' +response['cart'].qty + ' - '+size+'</p>\n' +
...
function removeCart(rowId){
    $.ajax({
        ...
        data:{rowId: rowId },
        success:function(response){
            //handler cart hover (master-page)
            $('.cart-count').text(response['count']);
            $('.cart-price').text(response['total']);
            $('.select-total h5').text('$'+response['total']);
